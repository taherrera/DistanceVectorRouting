...
void createroutertlv(RouterSet* aRouterSet, LinkSet* aLinkSet, unsigned char* tlv)
{
	...
	static unsigned char IDmask[4] = {0xf8,0x00,0x00,0x00};
	...
	// [MAXROUTERS+1+1+1+4]
	//memcpy(&tlv			,&linkqualroutedata,	MAXROUTERS);
	//memcpy(&tlv[MAXROUTERS]		,&IDmask,		(4));
	//memcpy(&tlv[MAXROUTERS+4]	,&IDseq,		(1));
	//memcpy(&tlv[MAXROUTERS+4+1]	,&length,		(1));
	//memcpy(&tlv[MAXROUTERS+4+1+1]	,&type,			(1));
	/*
